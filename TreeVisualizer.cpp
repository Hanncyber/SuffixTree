...
void TreeVisualizer::calculatePositions() {
    ...
    // Adjust size based on tree structure
...
void TreeVisualizer::drawEdge(QPainter& painter, int x1, int y1, int x2, int y2, const QString& label) {
    ...
    // Draw edge label on a colored background
    if (!label.isEmpty()) {
        int midX = (x1 + x2) / 2;
        int midY = (y1 + y2) / 2;
...
QColor TreeVisualizer::getNodeColor(SuffixNode* node) {
    if (node->suffix_index != -1) {
        // Leaf nodes - use rainbow colors based on suffix index
        int hue = (node->suffix_index * 137) % 360;  // Golden angle for good distribution
